...
void* GetInterface(HMODULE dllBase, const char* name)
{
    ...
    tCreateInterface CreateInterface = (tCreateInterface)GetProcAddress(dllBase, "CreateInterface");
...
bool Trace(IEngineTrace* EngineTrace, DWORD localPlayer, DWORD entity, Vector3 from, Vector3 to)
{
    CGameTrace trace;
    Ray_t ray;
    ...
    if ((DWORD)trace.hit_entity == entity)
    {
        return true;
    }
    return false;
